...
docker volume create pg_auto_failover1
docker stop pg_auto_failover1 || echo $?
docker rm pg_auto_failover1 || echo $?
docker run \
    --detach \
    ...
    --env PG_AUTOCTL_MONITOR=postgres://autoctl_node@pg_auto_failover0:5432/pg_auto_failover?sslmode=prefer \
    --env PG_AUTOCTL_NAME=pg_auto_failover1 \
    ...
    --env USER_ID="$(id -u)" \
    --hostname pg_auto_failover1 \
    --mount type=bind,source=/etc/certs,destination=/etc/certs,readonly \
    --mount type=volume,source=pg_auto_failover1,destination=/var/lib/postgresql \
    --name pg_auto_failover1 \
    --network name=docker \
    --restart always \
